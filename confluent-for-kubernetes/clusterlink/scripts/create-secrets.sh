...
kubectl create secret generic credential \
  --from-file=plain-users.json=$MY_HOME/secrets/creds-kafka-sasl-users.json \
  --from-file=plain.txt=$MY_HOME/secrets/creds-client-kafka-sasl-user.txt # \
 # --from-file=basic.txt=$MY_HOME/secrets/creds-basic-users.txt

kubectl create secret generic rest-credential \
  --from-file=basic.txt=$MY_HOME/secrets/rest-credential.txt

kubectl create secret generic password-encoder-secret \
  --from-file=password-encoder.txt=$MY_HOME/secrets/passwordencoder.txt

kubectl create secret generic restclass-ccloud --from-file=basic.txt=$MY_HOME/secrets/basic-cloud.txt

kubectl create secret generic jaasconfig-ccloud --from-file=plain-jaas.conf=$MY_HOME/secrets/cloud-jaas.text

kubectl create secret generic s3-credentials \
  --from-file=custom.properties=$MY_HOME/secrets/aws-secrets.txt